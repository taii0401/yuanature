...
class CreateProductFreeTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('product_free', function (Blueprint $table) {
            $table->id();
            $table->string('uuid',50)->comment('uuid');
            $table->integer('product_id')->nullable()->comment('商品ID');
            $table->integer('product_amount')->nullable()->comment('商品數量');
            $table->string('category')->nullable()->comment('商品分類：web_code.type = product_category');
    ...
    {
        Schema::dropIfExists('product_free');
    }
}
